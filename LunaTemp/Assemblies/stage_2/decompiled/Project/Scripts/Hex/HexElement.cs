...
namespace Project.Scripts.Hex
{
	public class HexElement : MonoBehaviour
	{
		...
		{
			base.transform.DOPath(path, duration, PathType.CatmullRom).SetEase(Ease.OutQuad);
			base.transform.DORotateQuaternion(targetRotation, duration).SetEase(Ease.OutQuad);
		}

		public Tweener Disappear(float duration)
		{
			return base.transform.DOScale(Vector3.zero, duration).OnComplete(delegate
			{
